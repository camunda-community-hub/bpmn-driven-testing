...
gradle clean build
  [Tags]  xignore  gradle
  ${result}=  Run process
  ...  gradle  -p  ${CURDIR}/advanced-spring-junit5  clean  build  -Pplugin.version\=${VERSION}  --info  --stacktrace
  ...  shell=True  stdout=${TEMP}/advanced-spring-junit5.out  stderr=STDOUT
...
Assert Test Code Generation
  ...
  ${testSources}  Set variable  ${CURDIR}/advanced-spring-junit5/${buildDir}/bpmndt
