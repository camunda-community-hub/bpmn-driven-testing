...
public class HandleAsyncAfter implements Function<BpmnNode, MethodSpec> {

  @Override
  public MethodSpec apply(BpmnNode node) {
    return MethodSpec.methodBuilder(String.format("%s_after", node.getLiteral()))
        .addJavadoc("Overwrite to assert the state after $L $L.", node.getType(), node.getId())
