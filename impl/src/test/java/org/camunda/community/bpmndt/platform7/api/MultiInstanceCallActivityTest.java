...
public class MultiInstanceCallActivityTest {
  ...
  public void testVerifyWithAssertionError() {
    handler.handle(0).verify((pi, callActivity) -> assertThat(callActivity.getDefinitionKey()).isEqualTo("not-equal"));
  ...
  private class TestCase extends AbstractJUnit5TestCase<TestCase> {
    ...
    protected InputStream getBpmnResource() {
      try {
        return Files.newInputStream(Platform7TestPaths.advancedMultiInstance("callActivity.bpmn"));
      } catch (IOException e) {
        return null;
      }
    }
