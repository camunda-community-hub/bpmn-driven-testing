...
public interface TestCaseActivity {
   ...
   * 
   * @return The event code or {@code null}, if the activity is not an error or escalation event or
   *         the code is not specified.
   */
  String getEventCode();
  ...
  /**
   * Determines how deep the activity is nested within the BPMN process. Level {@code 0}, means that
   * the activity is not nested in a scope (embedded sub process).
   * 
   ...
   * 
   * @return The topic name or {@code null}, if the activity is not an external task or the topic name
   *         is not specified.
   */
  String getTopicName();
