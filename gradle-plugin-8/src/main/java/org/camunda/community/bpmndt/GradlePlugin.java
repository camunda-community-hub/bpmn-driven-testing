...
public class GradlePlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    ...
    var extension = project.getExtensions().create(Constants.EXTENSION_NAME, GradleExtension.class);

    // add test source directory
    var buildPath = project.getLayout().getBuildDirectory().get().getAsFile().toPath();
    var testSourcePath = buildPath.resolve(Constants.EXTENSION_NAME);
    try {
    ...
    var testSourceSet = javaExtension.getSourceSets().getByName(SourceSet.TEST_SOURCE_SET_NAME);

    var additionalTestSourceSet = javaExtension.getSourceSets().create(SOURCE_SET_NAME);
    ...
    var generatorTask = project.getTasks().register(GeneratorTask.NAME, GeneratorTask.class, task -> {
